...
import { 
    ...
    LABEL_CLAMP_PADDING_PIXEL,  // 預設邊界空白 padding = 12
} from "../scene/ConsumerOptSceneBuilder";
...
export class ConsumerOptController {
    ...
    //
    // 設計目的：
    // - 讓 app 可以在切換 feature / unmount 時釋放 controller 內部資源
    // - 目前 ConsumerOptController 主要資源是「排程中的 flush」與「listeners」
    // - 不改現有行為：只是提供一個安全的清理入口
    // ------------------------------------------------------------
    dispose(): void {
        // 取消可能還排程中的 flush（避免 unmount 後還 build）
        this.cancelScheduledFlushIfAny();

        // 清空訂閱者，避免 memory leak
        this.listeners = [];

        // 清空 cache（dispose 後不再持有資料）
        this.lastScene = null;
        this.lastViewport = null;
    }
