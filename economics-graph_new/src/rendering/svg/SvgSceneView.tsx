...
import type { DragCallbacks, DragKind, DragState, PixelPoint } from "./types";
import { SVG_VIEW_STYLE, SVG_HIT_TEST_DEFAULTS } from "./constants";
import { getLocalSvgPoint, clampToRect } from "./svgDomCoords";
import { SvgTextNodeRegistry } from "./SvgTextNodeRegistry";
import { hitTestDraggable } from "./hitTest";
import { RafCoalescer } from "../raf/RafCoalescer";  // ✅ [CHANGED] use generic API (request/cancel)
import { SvgDrawableRenderer } from "./svgDrawableRenderer";

// ------------------------------------------------------------

// ------------------------------------------------------------
// PendingDrag：rAF coalescer 需要的「最後一次拖曳事件快照」
// - 同一個 frame 內可能多次 pointermove，只保留最後一次
// ------------------------------------------------------------
type PendingDrag = {
    id: string;
    kind: DragKind;
...
export class SvgSceneView extends React.Component<Props> {
    ...
    private raf: RafCoalescer<PendingDrag>;  // ✅ [CHANGED] generic coalescer
// ✅ [NEW] Renderer（可替換，為 D3/KaTeX/MathBox 預留）
    ...
    constructor(props: Props) {
        ...
        this.raf = new RafCoalescer<PendingDrag>();  // ✅ [CHANGED] generic API
        this.renderer = new SvgDrawableRenderer(this.textRegistry);
        // bind
        this.commitPendingDrag = this.commitPendingDrag.bind(this);  // ✅ [CHANGED] keep stable reference
    ...
    // commitPendingDrag
    // 設計目的：
    // - RafCoalescer 在下一個 frame 只 commit 一次
    // - 這裡把 coalesced 的 PendingDrag 轉成真正的 callbacks 呼叫
    //
    // Input：next（最後一次拖曳事件快照）
    // Output：void
    // ----------------------------------------------------------
    private commitPendingDrag(next: PendingDrag): void {
        this.flushDragToCallbacks(next.id, next.kind, next.p);
    }
...
// flushDragToCallbacks
    ...
    private handlePointerDown(e: React.PointerEvent<SVGGElement>): void {
        ...
        const hit = hitTestDraggable(drawables, p, SVG_HIT_TEST_DEFAULTS, this.textRegistry);
        if (!hit) {
        return;
        }
    ...
    private handlePointerMove(e: React.PointerEvent<SVGGElement>): void {
        ...
        const pending: PendingDrag = { id: draggingId, kind: draggingKind, p };
        this.raf.request(pending, this.commitPendingDrag);  // ✅ [CHANGED] generic API
    }
